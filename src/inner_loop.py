...
def build_classifier_usage_fields(
    ...
    summary = {
        "provider": {
            ...
            "completion_tokens": int(llm_usage.get("provider_completion_tokens") or 0),
            "total_tokens": int(llm_usage.get("provider_total_tokens") or 0),
            "calls": int(llm_usage.get("provider_calls") or 0),
        },
        "estimated": {
            "prompt_tokens": int(llm_usage.get("estimated_prompt_tokens") or 0),
            "completion_tokens": int(llm_usage.get("estimated_completion_tokens") or 0),
            "total_tokens": int(llm_usage.get("estimated_total_tokens") or 0),
            "calls": int(llm_usage.get("estimated_calls") or 0),
        },
        "cached": {
            "prompt_tokens": int(llm_usage.get("cached_prompt_tokens") or 0),
            "completion_tokens": int(llm_usage.get("cached_completion_tokens") or 0),
            "total_tokens": int(llm_usage.get("cached_total_tokens") or 0),
            "calls": int(llm_usage.get("cached_calls") or 0),
        },
    }
    ...
    return {
        ...
        "classifier_prompt_tokens": input_tokens,
        "classifier_completion_tokens": output_tokens,
        "classifier_total_tokens": total_tokens,
        "classifier_provider_prompt_tokens": summary["provider"]["prompt_tokens"],
        "classifier_provider_completion_tokens": summary["provider"]["completion_tokens"],
        "classifier_provider_total_tokens": summary["provider"]["total_tokens"],
        "classifier_estimated_prompt_tokens": summary["estimated"]["prompt_tokens"],
        "classifier_estimated_completion_tokens": summary["estimated"]["completion_tokens"],
        "classifier_estimated_total_tokens": summary["estimated"]["total_tokens"],
        "classifier_cached_prompt_tokens": summary["cached"]["prompt_tokens"],
        "classifier_cached_completion_tokens": summary["cached"]["completion_tokens"],
        "classifier_cached_total_tokens": summary["cached"]["total_tokens"],
        "classifier_usage_source_summary": summary,
        "memory_context_chars_proxy": avg_context_len,
    }
    ...
    for batch_start in range(0, len(examples), batch_size):
        ...
        for idx, ex, pred, meta, prompt_info, _predict_s in pred_results:
            ...
            result = {
                "input": inp,
                "prediction": pred,
                "ground_truth": _normalize_target(tgt),
                "was_correct": ok,
                "metadata": meta,
            }
            if metrics:
...
if __name__ == "__main__":
    ...
    classifier_model = classifier_config(cfg)
    model = args.model or classifier_model["model"]
    api_base = args.api_base if args.api_base is not None else classifier_model.get("api_base")
    api_key = classifier_model.get("api_key")
    llm = LLM(
        model=model, api_key=api_key, api_base=api_base, temperature=args.temperature
    )
    ...
    classifier_usage_fields = build_classifier_usage_fields(llm_usage, avg_context_len)
